...
describe('parser', () => {
  ...
  it('interpolation in element', () => {
    const ast = parse('<h1>{{msg}}</h1>', baseOptions) as ASTElement
    expect(ast.tag).toBe('h1')
    expect(ast.plain).toBe(true)
    const node = ast.children[0] as ASTExpression
    expect(node.expression).toBe('_s(msg)')
  })
  ...
  it('child elements', () => {
    ...
    expect(ast.plain).toBe(true)
    const node = ast.children[0] as ASTElement
    expect(node.tag).toBe('li')
    expect(node.plain).toBe(true)
  ...
  it('svg element', () => {
    ...
    expect(ast.plain).toBe(true)
    const node = ast.children[0] as ASTText
